...
#'
#' This function fits an Identifiable Variational Autoencoder (iVAE) model where the latent variables follow an AR(R) process, R being the autoregressive order. It also supports handling auxiliary data and accommodates various choices for source and error distributions.
#'
...
#' @param prev_data_list A list of numeric matrices of previously observed data, used for constructing the AR(R) latent prior.
...
iVAEar <- function(data, aux_data, latent_dim, prev_data_list, prev_aux_data_list, hidden_units = c(128, 128, 128), aux_hidden_units = c(128, 128, 128),
  ...
  hist <- vae %>% fit(inputs, data_scaled, 
  ...
  iVAE_object <- list(
    IC_unscaled = IC_estimates, IC = IC_estimates_scaled, data_dim = p, latent_dim = latent_dim,
