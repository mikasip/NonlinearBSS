...
#' res <- VAE(mixed_data, p,
#'     hidden_units = c(128, 64, 32),
#'     lr_start = 0.0001, epochs = 1, batch_size = 64
#' )
#' @export
GMVAE <- function(data, latent_dim, K, hidden_units = c(128, 128, 128), validation_split = 0,
    ...
    for (i in 1:K) {
        ...
        sampled_z <- z_mean_and_var %>% sampling_layer()
        latent_samples <- append(latent_samples, sampled_z)
    }

    est_weights_x <- input
    for (n_units in hidden_units) {
        est_weights_x <- est_weights_x %>%
            layer_dense(units = n_units, activation = activation)
    }
    est_weights_x <- est_weights_x %>%
        layer_dense(units = K, acitvation = "softmax")

    weighted_sum_layer <- weighted_sum(p = p, k = K)
    z <- weighted_sum_layer(layer_concatenate(latent_samples), est_weights_x)

    input_decoder <- layer_input(latent_dim)
    decoder <- input_decoder
    for (n_units in rev(hidden_units)) {
        decoder <- decoder %>% layer_dense(
            units = n_units,
            activation = activation
        )
    }
    decoder <- decoder %>% layer_dense(units = p)
